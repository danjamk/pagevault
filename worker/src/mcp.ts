 ...
 *
 * Identical to `handleMcp` except it does NOT check the bearer itself: the OAuthProvider
 * has already validated an issued token before routing here, and the request carries the
 * granted operator identity on `ctx.props`. The static-bearer path stays in `handleMcp`
 * so Claude Code keeps working — this is the second, independent auth surface ADR-006
 * says `/mcp` should have, not a replacement for the first.
 */
